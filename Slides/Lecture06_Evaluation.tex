  ...
  \begin{columns}[T]
    \column{0.52\textwidth}
      \begin{itemize}\small
        ...
        \item \textbf{Asymmetric:} a 100\% overforecast produces
              MAPE = 100\%; a 100\% underforecast is impossible
              when $y>0$ (bounded at 100\% from below but not above)
...
\begin{frame}{Accuracy by Horizon}
  ...
  \begin{columns}[T]
    ...
    \column{0.44\textwidth}
      \begin{keybox}
        Always report accuracy \textbf{by horizon}, not just
        as a single average.
      \end{keybox}
...
\begin{frame}{Walk-Forward Validation: Step-by-Step}
  \begin{columns}[T]
    ...
    \column{0.44\textwidth}
      ...
      \end{keybox}
      \muted{\footnotesize\itshape
        $e_{t,h}$ generalizes $e_{T+h}$ from Section~2 to multiple
...
\begin{frame}{DM Statistic and Mechanics}
  \begin{definitionbox}{Diebold-Mariano Test \parencite{Diebold1995}}
    {\small
    Let $d_t = g(e_{1t}) - g(e_{2t})$ be the \textbf{loss differential}
    (e.g., $d_t = e_{1t}^2 - e_{2t}^2$ for MSE loss). Then:
    \[
    ...
    \]
    $H_0$: equal predictive accuracy $\bigl(\E[d_t] = 0\bigr)$.\\[2pt]
    $\widehat{\operatorname{se}}(\bar{d})$ uses a \textbf{HAC variance estimate}
    to account for serial correlation in $d_t$.}
  \end{definitionbox}
  \begin{columns}[T]
  ...
  \begin{columns}[T]
    \column{0.54\textwidth}
      ...
      \[
        ...
        = \tfrac{1}{4}\bigl(\sigma_1^2 + 2\rho\sigma_{12} + \sigma_2^2\bigr)
      \]
      ...
      that weighted combinations \emph{consistently} outperform
      the best individual model in out-of-sample tests.
    \column{0.42\textwidth}
      \begin{keybox}

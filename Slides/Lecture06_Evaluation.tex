...
\begin{frame}{Forecast Accuracy Metrics}
  ...
  \begin{columns}[T]
    ...
    \column{0.46\textwidth}
      ...
      \end{keybox}
  \end{columns}
  \muted{\footnotesize\parencite{Hyndman2006}\quad\itshape
  ...
  \begin{columns}[T]
    ...
    \column{0.44\textwidth}
      \begin{keybox}
        ...
        $\MASE < 1$ $\Rightarrow$ beats seasonal na\"{i}ve\\
        $\MASE > 1$ $\Rightarrow$ worse than na\"{i}ve\\[2pt]
...
\begin{frame}{Walk-Forward Validation: Step-by-Step}
  \begin{columns}[T]
    \column{0.52\textwidth}
      ...
      \end{enumerate}
    \column{0.44\textwidth}
      \textit{Walk-forward errors are the closest thing to a \textbf{live
      forecast track record}: each $e_{t,h}$ represents the error you
      would have made using this model in real time.}
      \vspace{0.1cm}
      \muted{\footnotesize\itshape
        ...
        Number of usable origins at horizon $h$: $n_h = T - T_0 - h + 1$;
        at the maximum horizon $h=H$: $n_H = T - T_0 - H + 1$;
  ...
  \begin{columns}[T]
    ...
    \column{0.40\textwidth}
      ...
      \begin{itemize}\small
        ...
        \item Fail to reject $H_0$: insufficient evidence that models differ
              --- do not interpret as evidence of equality
      \end{itemize}
      \muted{\footnotesize\itshape
        $\bar{d} > 0$: SARIMA has
...
\begin{frame}{Why Combine Forecasts?}
  \begin{columns}[T]
    ...
    \column{0.42\textwidth}
      \textit{If two models have equal accuracy but errors are
      \textbf{not perfectly correlated}, the simple average

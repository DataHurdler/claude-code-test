  ...
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
      ...
      \[
        ...
        \quad \text{but} \quad
        \Var(\hat{\bm{\beta}}_{\mathrm{OLS}}) \text{ large}
      \]
      ...
      \[
        ...
        \quad \text{but} \quad
        \Var(\hat{\bm{\beta}}_{\lambda}) \text{ smaller}
      \]
    ...
    \begin{column}{0.42\textwidth}
      \begin{keybox}
        ...
        $\MSE = \mathrm{Bias}^2 + \Var + \sigma^2$\\[4pt]
...
% -----------------------------------------------------------------------------
  ...
  \muted{\footnotesize\itshape
    ...
    Elastic Net $\alpha$ is the L1/L2 mixing ratio --- distinct from ETS smoothing $\alpha$ (L03)
    and ECM speed-of-adjustment $\alpha$ (L05).
    The tuning of $\lambda$ (and $\alpha$ for Elastic Net) is covered in Section~5.}
\end{frame}
...
\sectionslide{LASSO Regression (L1)}{%
  Shrinks \emph{and} selects: L1 penalty sets irrelevant coefficients to exactly zero.}
...
\begin{frame}{Section Overview}
  \textbf{Goal:} select $\lambda^*$ that minimizes out-of-sample prediction error.
      ...
      \begin{enumerate}
        ...
        \item For each $\lambda$:
          \begin{itemize}
            ...
            \item Fit regularized model on each train fold
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.44\textwidth}
      ...
      \begin{keybox}
        ...
        regularized, more robust).
      \end{keybox}
    \end{column}
  \end{columns}
  \muted{\footnotesize\itshape When would the one-SE rule backfire?
    (Hint: consider a highly asymmetric validation curve.)}
\end{frame}
...
% -----------------------------------------------------------------------------
  \textbf{What survives LASSO regularization on RSXFS?}
...
% -----------------------------------------------------------------------------
\begin{frame}{Forecast Comparison: SARIMA vs.\ Regularized Models}
% -----------------------------------------------------------------------------
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
    ...
    \begin{column}{0.44\textwidth}
      \textbf{Takeaways:}
      \begin{itemize}
        \item All regularized models beat SARIMA on this feature set
        ...
        \item Larger gains expected in \textbf{multi-series} settings
              (shared regularization) or when many external regressors exist
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
...
% -----------------------------------------------------------------------------
  \begin{keybox}
    \begin{enumerate}
      \item \textbf{OLS instability} with many/correlated predictors motivates
            regularization — the bias--variance tradeoff at work.
  ...
  capture nonlinearities that penalized linear models cannot.
\end{frame}

...
% -----------------------------------------------------------------------------
  ...
  update one $\beta_j$ at a time, holding others fixed, applying a
  \emph{soft-thresholding} operator:
  \[
    \hat{\beta}_j \leftarrow
    \mathrm{sign}(z_j)\,\max\!\left(|z_j| - \tfrac{\lambda}{2},\, 0\right)
  \]
  where $z_j = \mathbf{x}_j^\top(\mathbf{y} - \mathbf{X}_{-j}\hat{\bm{\beta}}_{-j})$
  is the partial residual inner product for predictor $j$.

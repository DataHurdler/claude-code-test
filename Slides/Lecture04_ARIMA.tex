...
\begin{frame}{The Unit Root}
  ...
  \begin{warningbox}
    With a unit root, the \textbf{effect of every past shock is permanent}.
...
\begin{frame}{Complementary Test: KPSS}
  The ADF and KPSS (Kwiatkowski--Phillips--Schmidt--Shin) tests address \emph{opposite} null hypotheses:
  \begin{center}
    {\small
    \begin{tabular}{lll}
      \toprule
...
\begin{frame}{Box-Jenkins Methodology \parencite{BoxJenkins2015}}
  \begin{center}
  ...
  Ljung-Box $H_0$: white noise \muted{\footnotesize(test at lag $L=\min(10,T/5)$;
  $p$-value $<0.05$ signals residual autocorrelation --- re-identify)}
\end{frame}
...
\begin{frame}{Forecasting with ARIMA}
  ...
  \begin{keybox}
    ...
    prediction interval width scales as $\sigma\sqrt{h}$ for ARIMA(0,1,0).
...
\begin{frame}{SARIMA($p$,$d$,$q$)($P$,$D$,$Q$)[$m$]}
  Monthly retail sales have autocorrelation at lag 12, 24, \ldots\ ---
  beyond standard ARIMA. Denote the AR and MA lag polynomials
  $\phi_p(B)\!=\!1{-}\phi_1 B{-}\cdots{-}\phi_p B^p$ and
  $\theta_q(B)\!=\!1{+}\theta_1 B{+}\cdots{+}\theta_q B^q$;
  SARIMA adds seasonal counterparts $\Phi_P(B^m)$, $\Theta_Q(B^m)$:
...
\begin{frame}{What's Next: Multivariate Methods}
  We now have four model families in our toolkit:
  \begin{itemize}\small

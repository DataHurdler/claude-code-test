...
\begin{frame}{All 11 Methods on RSXFS}
  ...
  \vspace{0.05cm}
  \muted{\footnotesize\itshape
    Equal-weight combination (RMSE $2{,}080$, MAE $1{,}530$) nearly matches
    XGBoost (RMSE $2{,}050$, MAE $1{,}510$) and LSTM (RMSE $1{,}920$, MAE $1{,}410$)
    at a fraction of the deployment complexity.}
  \muted{\footnotesize\itshape Which gaps are statistically confirmed? DM test results on the next slide.}
\end{frame}
...
\begin{frame}{Recommended Deployment Pipeline for RSXFS}
  {\small For production, the three-model combination (SARIMA $+$ XGBoost $+$ LSTM)
  is simplified to two models, dropping LSTM to eliminate GPU dependency.
  The RMSE difference is modest.}
  \vspace{0.1cm}
  \begin{keybox}
    ...
    \begin{enumerate}\setlength{\itemsep}{3pt}
      ...
      \item \textbf{Form equal-weight combination}:
            $\yhat_t = \tfrac{1}{2}(\yhat_t^{\text{SARIMA}}
            + \yhat_t^{\text{XGB}})$.
  ...
  \end{keybox}
  \muted{\footnotesize\itshape
    LSTM alone (RMSE $1{,}920$) beats the combination by 160 units
    but adds GPU infrastructure and refit complexity not justified for most business settings.}
\end{frame}
    ...
    \column{0.54\textwidth}
      ...
      \begin{itemize}\small
        ...
        \item \textbf{Seasonality:} extreme (6$\times$ winter/summer ratio),
              virtually no AR structure beyond lag 12 (the seasonal period, $m = 12$)
...
\begin{frame}{RESGAS Leaderboard: Framework Prediction vs.\ Results}
  ...
  \end{columns}
  \smallskip
  \muted{\footnotesize\itshape SARIMA wins partly because COVID shifted residential
    usage patterns that ML could not anticipate.
    Is that an advantage of SARIMA --- or a limitation of the test period?}
\end{frame}

...
% -----------------------------------------------------------------------------
  \begin{keybox}
    \textbf{Lecture 10 best result (36 features):}
    LSTM, RMSE $\approx$ 1,920 --- down from XGBoost's 2,250
    using only 26 features.
  ...
  \medskip
  \textbf{Today's goal:} build \texttt{make\_features\_extended()}
...
\begin{frame}{Section Overview}
  ...
  \texttt{df['value'].shift(1)} to prevent leakage (see Slide 6).
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      ...
      \begin{tabular}{lll}
        ...
        \midrule
        \texttt{roll\_mean\_w} & $\bar{y}_{t-1:t-w}$
                               & Local trend \\
        \texttt{roll\_std\_w}  & $s_{t-1:t-w}$
                               & Volatility \\
      ...
      \[
        \mathrm{EWM}_t
          ...
          \quad \alpha \in (0,1)
      \]
      With $\alpha = 0.3$: recent values count
      $3\times$ more than values 4 months back.
    \end{column}
    \begin{column}{0.44\textwidth}
...
% -----------------------------------------------------------------------------
  ...
  \begin{warningbox}
    ...
    the sequence itself; adding 11 redundant dummies adds noise.
...
\begin{frame}{Section Overview}
  ...
  \begin{itemize}
    \item \textbf{Year-over-year (YoY) change:}
          $\Delta^{(12)} y_t = y_t / y_{t-12} - 1$
    \item \textbf{Month-over-month (MoM) change:}
          $\Delta^{(1)} y_t = y_t / y_{t-1} - 1$
...
def make_features_extended(df,
    ...
    dums = pd.get_dummies(
        X['month'], prefix='mo', drop_first=True)
    X = pd.concat([X, dums], axis=1)
    # Year-over-year and month-over-month ratios
    X['yoy'] = X['value'].shift(1) / X['value'].shift(13) - 1
    X['mom'] = X['value'].shift(1) / X['value'].shift(2) - 1
    return X.dropna()   # 36 features after dropna
\end{lstlisting}
\end{frame}
...
\begin{frame}{Section Overview}
  ...
  \begin{enumerate}
    \item \textbf{LASSO} (from Lecture 08): $\ell_1$ penalty
          ...
          Implicit selection via regularization path
...
% -----------------------------------------------------------------------------
\begin{frame}{Feature Selection via LASSO and Permutation Importance}
% -----------------------------------------------------------------------------
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      \textbf{LASSO path (Lecture 08 connection):}
      \[
        \hat{\boldsymbol{\beta}}^{\text{LASSO}}
          = \operatorname{arg\,min}_{\boldsymbol{\beta}}
            \sum_{t} (y_t - \mathbf{x}_t^\top \boldsymbol{\beta})^2
            + \lambda \|\boldsymbol{\beta}\|_1
      \]
      As $\lambda$ increases:
      \begin{itemize}
        \item Lag 12 and lag 1 coefficients survive
              to large $\lambda$ --- high importance
        \item Rolling std features shrink early ---
              lower importance
        \item LASSO path implicitly ranks features by
              their marginal contribution to RMSE
      \end{itemize}
    ...
    \begin{column}{0.46\textwidth}
      ...
      \begin{enumerate}
        ...
        \item Record validation RMSE: $\text{RMSE}_0$
        \item For feature $j$: randomly shuffle column $j$,
              re-predict, record $\text{RMSE}_j$
        \item Importance$_j = \text{RMSE}_j - \text{RMSE}_0$
      \end{enumerate}

      \smallskip
      \textbf{Advantages over split-count importance:}
      \begin{itemize}
        \item Works on validation set --- not biased toward
              high-cardinality features \parencite{Molnar2022}
        \item Model-agnostic: applies to LSTM, XGBoost, etc.
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.49\textwidth}
      \begin{definitionbox}{RFECV Algorithm}
        ...
        \end{enumerate}
        \textbf{Select:} the subset size that maximises
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.40\textwidth}
      ...
      \begin{keybox}
        ...
        \textbf{LSTM 36f $<$ XGBoost 36f $<$
        LSTM 26f} would indicate the feature gap

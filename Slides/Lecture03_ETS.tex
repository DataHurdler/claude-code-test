...
\begin{frame}{The Exponential Weighting Idea}
  ...
  \begin{keybox}
    ...
    \textbf{forgotten}.
    \quad \pos{$\alpha \approx 1$}: almost all weight on the latest value.
    \quad \pos{$\alpha \approx 0$}: long memory.
  \end{keybox}
\end{frame}
...
\begin{frame}{The Error-Correction Form}
  ...
  \muted{\footnotesize\itshape (Recall from Lecture 2: SES shares the AR(1) error-correction structure, but requires no stationarity.)}
  \begin{keybox}
...
\begin{frame}{Estimating $\alpha$ and the SES Forecast}
  \textbf{Parameter estimation:} choose $\alpha$ and $\ell_0$ to minimise
  the sum of squared one-step-ahead errors \parencite[][Ch.~8]{Hyndman2021}:
  \[
    \min_{\alpha,\,\ell_0}\;\sum_{t=1}^{T}(y_t - \ell_{t-1})^2,
    \qquad 0 < \alpha \leq 1
  \]
  Solved numerically (bounded nonlinear optimisation).
...
\begin{frame}{Holt's Linear Method: Level and Trend}
  ...
  \end{columns}
  \muted{\footnotesize\itshape
    SES is nested inside Holt: set $\beta^* = 0$ and $b_0 = 0$.}
\end{frame}
...
\begin{frame}{Estimation and Initialization}
  ...
  \end{itemize}
  \begin{warningbox}
    Holt-Winters requires at least $\mathbf{2m}$ observations to initialise.
...
\begin{frame}{State Space Representation}
  Every ETS model can be written as a \textbf{state space model}:
  \[
...
\begin{frame}{Automatic Model Selection via AIC}
  ...
  \[
    ...
    \qquad \hat{\ell} = \text{maximized log-likelihood}
  \]
  \begin{center}
    {\small
    \begin{tabular}{lll}
...
\begin{frame}{Key Takeaways}
  \begin{keybox}
    ...
    \begin{enumerate}
      ...
      \item \textbf{SES}: one parameter, flat forecast ---

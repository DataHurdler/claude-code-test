...
\begin{frame}{Four Gaps ML Fills}
  \begin{columns}[T]
...
\sectionslide{The Bias-Variance Tradeoff}{%
  Every modeling decision shifts error between bias and variance.
  This is the central tension in all of statistical learning.}

% --- Slide: Formal decomposition (Slide 1 of 2) -------------------------------
\begin{frame}{MSE Decomposition}
  \begin{definitionbox}{Bias-Variance Decomposition}
    {\small
    Let $\hat{f}$ be an estimated model, $f$ the true function,
    and $x_0$ a new observation.
    \[
      \MSE(\hat{f}(x_0))
      = \bigl(\operatorname{Bias}[\hat{f}(x_0)]\bigr)^2
        + \Var[\hat{f}(x_0)] + \sigma^2
    \]
    \muted{\footnotesize\parencite{Hastie2009}}
    }
  \end{definitionbox}
  \vspace{0.15cm}
  Three distinct sources of error --- each unpacked on the next slide.\\[3pt]
  {\small Two of the three are \textbf{controllable} by the modeler.
  One is fixed by the data-generating process.}
  \vspace{0.1cm}
  \begin{center}
    \muted{\footnotesize\itshape (Each term unpacked on the next slide.)}
  \end{center}
\end{frame}

% --- Slide: MSE Decomposition: Each Term (Slide 2 of 2) ----------------------
\begin{frame}{MSE Decomposition: Each Term}
  \begin{columns}[T]
    \column{0.54\textwidth}
      \begin{itemize}\small
        \item \textbf{Bias\textsuperscript{2}} (underfitting): systematic
              error from wrong model class.\\
              {\footnotesize Example: $\text{Bias}^2 \approx 0.42$. Reduce by
              increasing model complexity.}
        \item \textbf{Variance} (overfitting): sensitivity to the
              particular training sample.\\
              {\footnotesize Example: $\Var \approx 0.31$. Reduce by
              regularization or more data.}
        \item \textbf{$\sigma^2$} (irreducible): determined solely by the
              DGP; neither better models nor additional observations
              can eliminate it.\\
              {\footnotesize Example: $\sigma^2 = 0.07$ --- fixed.}
      \end{itemize}
    \column{0.42\textwidth}
      \begin{keybox}
        {\small \textbf{Two levers you control:}\\[3pt]
        Complexity $\uparrow$:\\
        \hspace{0.5em}$\text{Bias}^2 \downarrow$,\;$\Var \uparrow$\\[3pt]
        Regularization $\uparrow$:\\
        \hspace{0.5em}$\text{Bias}^2 \uparrow$,\;$\Var \downarrow$\\[5pt]
...
\begin{frame}{Data Leakage: Definition and Examples}
  ...
  \begin{columns}[T]
    ...
    \column{0.44\textwidth}
      ...
      \end{warningbox}
      \muted{\footnotesize\itshape Socratic: a practitioner fits \texttt{StandardScaler}
      on the full series before splitting.
      What is the first production sign that something went wrong?}
  \end{columns}
\end{frame}
...
\begin{frame}{CV in Practice: Hyperparameter Tuning}
  {\small A \textbf{hyperparameter} (e.g., $\lambda$) controls model complexity
  but is \emph{not} estimated from data --- it is chosen before fitting.
  CV selects the best value.}\vspace{4pt}
  \begin{columns}[T]
    \column{0.52\textwidth}
      \begin{enumerate}\small
        ...
        \item Select $\lambda^*$ minimizing mean CV RMSE
    ...
    \column{0.44\textwidth}
      \begin{examplebox}{LASSO on RSXFS}
        ...
        Test RMSE with $\lambda^* = 0.1$: $1{,}810$
        (close to CV RMSE, as expected).}
      \end{examplebox}
  \end{columns}
\end{frame}
...
\begin{frame}{Ridge, LASSO, and Elastic Net}
  \begin{definitionbox}{Penalized Regression}
    {\small
    \[
      \bhat = \arg\min_\beta
    ...
    \quad \textbf{EN \parencite{ZouHastie2005}:}
    $\alpha\lVert\beta\rVert_1 + (1{-}\alpha)\lVert\beta\rVert_2^2$
    \muted{\footnotesize\hfill\parencite{Hastie2009}}
    }
  \end{definitionbox}
  \begin{columns}[T]
    \column{0.54\textwidth}
    ...
    \column{0.42\textwidth}
      \muted{\footnotesize
        ...
        Note: EN $\alpha$ is the LASSO/Ridge mixing ratio
        (not ETS smoothing $\alpha$ from L03).\\[3pt]
...
\begin{frame}{From Time Series to Feature Matrix}
  ...
  We extract temporal patterns as columns.\\[2pt]
  {\small \muted{Unlike ARIMA, ML algorithms assume i.i.d.\ rows:
  they cannot exploit autocorrelation internally.
  We must encode temporal structure as explicit columns.}}
  \begin{center}
    {\footnotesize
    ...
    \column{0.56\textwidth}
      \begin{keybox}
        ...
        \begin{enumerate}
          ...
          \item $\MSE = \operatorname{Bias}^2 + \Var
                + \sigma^2$ governs every modeling decision.

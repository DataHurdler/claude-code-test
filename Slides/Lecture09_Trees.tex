...
% -----------------------------------------------------------------------------
\begin{frame}{Motivation: Beyond Linear Models}
% -----------------------------------------------------------------------------
  \begin{keybox}
    \textbf{Lecture 08 best result:} Elastic Net, RMSE $\approx$ $2{,}540$ on RSXFS.\\[4pt]
    Elastic Net is a linear model: $\hat{y} = \mathbf{x}^\top \hat{\bm{\beta}}$.
    It cannot capture \emph{interactions} or \emph{threshold effects}
    (e.g., the holiday effect amplifies when the economy is strong).
  \end{keybox}

  \medskip
  \textbf{What tree-based methods add:}
  \begin{itemize}
    \item \textbf{Nonlinearity:} predictions are piecewise constant over feature regions
    \item \textbf{Interactions:} splits on $x_1$ in a region defined by $x_2$ capture
          $x_1 \times x_2$ interactions automatically
    \item \textbf{No standardization required:} scale-invariant splits
  \end{itemize}

  \medskip
  \textbf{Today's goal:} build from a single decision tree
  to Random Forests and XGBoost, then compare against the Lecture 08 leaderboard.
\end{frame}
...
\sectionslide{Decision Trees: CART}{Building blocks of ensemble methods through recursive binary splits of the feature space}

% -----------------------------------------------------------------------------
\begin{frame}{Recursive Binary Splitting}
% -----------------------------------------------------------------------------
  \textbf{CART (Classification and Regression Trees) for regression:} at each node, find the split $(j, t)$ minimizing
...
\sectionslide{Bagging and Random Forests}{Variance reduction through bootstrap aggregation and random feature decorrelation}

% -----------------------------------------------------------------------------
\begin{frame}{Bagging: Bootstrap Aggregating}
% -----------------------------------------------------------------------------
  \begin{definitionbox}{Bagging}
    {\footnotesize Draw $B$ bootstrap samples; fit one deep tree on each; average:
    \vspace{-6pt}
    \[ \hat{y}_{\mathrm{bag}} = \tfrac{1}{B}\sum_{b=1}^{B} \hat{f}^{(b)}(\mathbf{x}) \]
    \vspace{-8pt}
    \muted{\footnotesize\parencite{Breiman1996}}}
  \end{definitionbox}
  ...
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      ...
      \vspace{-8pt}
      $\rho$ = pairwise prediction correlation;
      $\sigma^2$ = single-tree variance.\\[1pt]
      As $B \to \infty$, variance $\to \rho\sigma^2$ (not zero).
      Reducing $\rho$ matters as much as increasing $B$
    ...
    \begin{column}{0.52\textwidth}
      \begin{itemize}
        \item Default: $m = \lfloor\sqrt{p}\rfloor$ (sklearn $\geq$ 1.1);
              $\lfloor p/3 \rfloor$ recommended for regression \parencite[§15.3]{Hastie2009}
...
\sectionslide{Gradient Boosting and XGBoost}{Sequential residual fitting with second-order optimization and explicit regularization}
...
% -----------------------------------------------------------------------------
  \begin{definitionbox}{Gradient Boosting Algorithm}
    ...
    \begin{itemize}
      \item Residuals: $r_i^{(b)} = y_i - F_{b-1}(\mathbf{x}_i)$
            \muted{\footnotesize(negative gradient for squared-error loss)}
...
% -----------------------------------------------------------------------------
\begin{frame}{XGBoost: Three Advances Over GBM}
% -----------------------------------------------------------------------------
  ...
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
      \begin{itemize}
        \item \textbf{1. Newton step (2nd-order):} Uses gradient \emph{and}
              curvature of the loss to compute a more precise tree-fitting direction
              than first-order GBM.
        \item \textbf{2. Explicit regularization:} Penalizes leaf count and leaf
              weights directly in the objective — shrinks trees without relying on
              early stopping alone.
        \item \textbf{3. Column subsampling:} Like RF, randomly samples features
              per tree \emph{and} per split — reduces correlation and overfitting.
      \end{itemize}
    \end{column}
    \begin{column}{0.40\textwidth}
      \begin{keybox}
        Newton step $\Rightarrow$ more precise gradient direction.\\[2pt]
        Regularization $\Rightarrow$ guards against overfitting.\\[2pt]
        Together: XGBoost outperforms vanilla GBM on most benchmarks \parencite{Chen2016}.
      \end{keybox}
      \muted{\footnotesize\itshape Mathematical formulation: next slide.}
    \end{column}
  \end{columns}
\end{frame}

% -----------------------------------------------------------------------------
\begin{frame}{XGBoost: Optimization Objective}
% -----------------------------------------------------------------------------
  \muted{\footnotesize\itshape
    Here $f_b(\mathbf{x})$ is the output of tree $T_b$ in boosting step $b$
    --- same tree, XGBoost notation.}

  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
      \textbf{1. Newton-step objective (stage $b$):}
      \[
        \mathcal{L}^{(b)} = \sum_{i}
      ...
      \smallskip
      \textbf{2. Regularization term:}
      \[
        \Omega(f_b) = \gamma T + \tfrac{1}{2}\lambda \|\mathbf{w}\|_2^2
      \]
      $T$ = leaf count, $\mathbf{w}$ = leaf weights,
      $\gamma$ = per-leaf complexity penalty.
    \end{column}
    \begin{column}{0.40\textwidth}
      \textbf{Notation note:}\\[4pt]
      $\lambda$ here is XGBoost's L2 leaf-weight penalty.
      It is distinct from the regularization $\lambda$ (penalty strength)
      in Lecture 08.\\[8pt]
      \textbf{Practical effect:} Setting $\lambda > 0$ and $\gamma > 0$
      produces sparser, more interpretable trees even without
...
% -----------------------------------------------------------------------------
  ...
  \end{tabular}
  \end{center}
  \muted{\footnotesize\textsuperscript{*}sklearn API name;
    native API uses \texttt{num\_boost\_round}.}
  ...
  \vspace{-2pt}
  \muted{\footnotesize\itshape
    Socratic: if \texttt{learning\_rate} is halved, roughly how many additional
    trees are needed to maintain the same training loss?
    What does this imply for early stopping budget?}
\end{frame}
...
\sectionslide{Feature Importance}{Impurity, permutation, and XGBoost gain measures for model interpretation}

% -----------------------------------------------------------------------------
\begin{frame}{Feature Importance: Impurity and Permutation}
% -----------------------------------------------------------------------------
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      \begin{definitionbox}{1. Impurity-Based (RF Default)}
        \footnotesize
        \[
          \operatorname{Imp}(j)
          = \sum_{\text{nodes using } j}
            \Delta\operatorname{RSS} \cdot \frac{n_{\text{node}}}{n}
        \]
      ...
      \smallskip
      \muted{\footnotesize\itshape
        Caution: biased toward high-cardinality features
        (continuous variables get more split opportunities than binary dummies).
        Do not use for final reporting.}
    \end{column}
    \begin{column}{0.44\textwidth}
      \begin{definitionbox}{2. Permutation Importance}
        \footnotesize
        For each feature $j$:
        \begin{enumerate}
          \item Shuffle column $j$ in the OOB set
          \item Record increase in RMSE
        \end{enumerate}
        Larger increase $\Rightarrow$ more important.\\[2pt]
        \textbf{Unbiased} (no cardinality bias).\\
        \textbf{Model-agnostic.} Use for reporting.
      \end{definitionbox}
    \end{column}
  \end{columns}
\end{frame}
...
% -----------------------------------------------------------------------------
\begin{frame}{Feature Importance: XGBoost Gain and Rankings}
% -----------------------------------------------------------------------------
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      \begin{definitionbox}{3. XGBoost Gain Importance}
        \footnotesize
        Average \emph{gain} (improvement in objective function)
        per split using feature $j$, across all trees.\\[2pt]
        More interpretable than impurity for boosted trees
        because it weights by actual objective reduction.
      \end{definitionbox}

      \medskip
      \textbf{Practical rule:}
      \begin{itemize}\footnotesize
        \item Use \textbf{permutation} importance for model reports and presentations
        \item Use \textbf{impurity} for quick diagnostics during training
        \item Use \textbf{XGBoost gain} when comparing feature contributions\\inside a boosted model
      \end{itemize}
    \end{column}
...
\sectionslide{Hyperparameter Tuning}{Time-series-aware cross-validation with TimeSeriesSplit}
...
tscv = TimeSeriesSplit(n_splits=5, gap=1)  # gap>=1 for 1-step-ahead
rf_grid = {
...
\sectionslide{Application to Forecasting}{Comparing Random Forest and XGBoost against SARIMA and Elastic Net on RSXFS}

% -----------------------------------------------------------------------------
\begin{frame}{Tree-Based Forecasting: Design Choices}
% -----------------------------------------------------------------------------
  \textbf{Same feature matrix as Lecture 08:}
  12 lags + 3 rolling windows + 11 month dummies = 26 features
  (one dummy omitted vs.\ L08 for comparability; trees do not require it).
    ...
    \begin{column}{0.50\textwidth}
      ...
      \begin{itemize}
        ...
        \item Handle irrelevant features gracefully
              (unused features do not appear in splits)
  ...
  \begin{warningbox}
    ...
    If $y_t$ trends upward beyond the training max,
  ...
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      ...
      \muted{\footnotesize\itshape Values are illustrative.
        Strategy: direct multi-step — one model trained per horizon $h = 1, \ldots, 24$.}
    \end{column}
    \begin{column}{0.44\textwidth}
      ...
      \muted{\footnotesize\itshape
        ...
        Does this guarantee trees will always win with more data?
        (Hint: consider the no-extrapolation warning.)}
    \end{column}
  \end{columns}
\end{frame}
...
% -----------------------------------------------------------------------------
  \begin{keybox}
    \begin{enumerate}
      ...
      \item \textbf{RF vs.\ XGBoost:} prefer RF for speed and robustness with
            limited tuning; prefer XGBoost when maximum accuracy matters and
            you have time to tune \texttt{learning\_rate} and \texttt{max\_depth}.
    \end{enumerate}
  \end{keybox}

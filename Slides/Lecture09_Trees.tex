...
% -----------------------------------------------------------------------------
  ...
  \begin{itemize}
    \item \textbf{Nonlinearity:} predictions are piecewise constants over feature regions
...
% -----------------------------------------------------------------------------
  ...
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      ...
      \]
      $\rho$ = pairwise prediction correlation between trees;
      $\sigma^2$ = single-tree variance.\\[2pt]
...
% -----------------------------------------------------------------------------
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.42\textwidth}
      ...
      \end{keybox}
      \muted{\footnotesize\itshape
        Note: $\lambda$ here is XGBoost's L2 leaf-weight penalty
        (distinct from the regularization $\lambda$ in Lecture 08).}
    \end{column}
  \end{columns}
\end{frame}
...
% -----------------------------------------------------------------------------
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      ...
      \begin{warningbox}
        \footnotesize
        \textbf{Caution:} biased toward \emph{high-cardinality} features
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.46\textwidth}
      \begin{examplebox}{RSXFS Typical Rankings}
        ...
        \end{enumerate}
        Consistent across all three importance types — feature engineers should lead with lag\_12.
      \end{examplebox}
    \end{column}
  \end{columns}
\end{frame}

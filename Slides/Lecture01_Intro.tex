...
\begin{frame}{The Cost of Getting It Wrong}
  \begin{examplebox}{Real-World Forecast Failures}
    \begin{itemize}
      \item \textbf{UK NHS (2020):} Initial COVID hospitalization forecasts were off by 4$\times$ ---
      ...
      \item \textbf{Toys R Us (2017):} systematically over-ordered inventory based on outdated
            demand forecasts, contributing to bankruptcy.
      \item \textbf{Target Canada (2015):} Inventory system forecasting errors caused shelves
            to be overstocked on some items and empty on others --- a \$2B writedown.
    \end{itemize}
  \end{examplebox}
  \vspace{0.3cm}
  \begin{warningbox}
    ...
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}
        ...
        \item \key{High dimensionality:} More potential predictors than
              observations (especially in the ML half of the course).
      \end{itemize}
    \end{column}
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}
        ...
        \item Non-stationarity: the mean and variance may change over time.
...
\begin{frame}{Types of Forecasts}
  ...
  \begin{keybox}
    ...
    Density forecasting is beyond the scope of this course.
  \end{keybox}
\end{frame}
...
\begin{frame}{Stationarity: A First Look}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.48\textwidth}
      \begin{keybox}
        \small
        \textbf{Rule of thumb:} Visible trend or growing variance (heteroscedasticity)
  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      ...
      \begin{itemize}
        ...
        \item Limiting case: as $\alpha \to 1$ and $\beta \to 1$,
              Holt's method approaches the random walk with drift (Lecture 3)
      \end{itemize}
    \end{column}
...
\begin{frame}{Point Forecast Accuracy Metrics}
  ...
  and forecasts $\{\hat{y}_{T+1|T}, \ldots, \hat{y}_{T+H|T}\}$:
...
\begin{frame}{A Better Scale-Free Metric: MASE}
  ...
  \begin{itemize}
    ...
    \item $\text{MASE} > 1$: model is \emph{worse} than the seasonal na\"{i}ve benchmark

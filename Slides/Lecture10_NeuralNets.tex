...
\begin{frame}{Section Overview}
  \begin{keybox}
    \textbf{Feedforward networks} (multilayer perceptrons, MLPs) are the
    building block of all deep learning. Understanding how a single layer
    transforms inputs to outputs is prerequisite to understanding LSTM
    and attention.
  \end{keybox}
\end{frame}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.44\textwidth}
      \begin{definitionbox}{Activation Function}
        ...
        linear layer. Non-linearity is what gives neural networks their
      ...
      \begin{examplebox}{Why ReLU?}
        \footnotesize
        ReLU avoids vanishing gradients that plagued sigmoid/tanh in deep
        networks \parencite{Bengio1994}. Default choice for hidden layers.
      \end{examplebox}
    \end{column}
  \end{columns}
\end{frame}
...
% -----------------------------------------------------------------------------
  \begin{columns}[T]
    ...
    \begin{column}{0.44\textwidth}
      \begin{keybox}
        \footnotesize
        The key practical skills are NOT deriving backpropagation.
        They are: choosing architecture depth and width, setting the
        learning rate schedule, applying dropout for regularization,
        and recognizing overfitting early.
      \end{keybox}
...
\begin{frame}{Section Overview}
  \begin{keybox}
    \textbf{Recurrent networks} maintain a hidden state $\mathbf{h}_t$
    that is updated at each time step. This gives the model explicit
    memory of the sequence --- unlike feedforward networks that see
    only a fixed-length feature window.
  \end{keybox}
\end{frame}
...
% -----------------------------------------------------------------------------
  \begin{columns}[T]
    \begin{column}{0.54\textwidth}
      \begin{definitionbox}{Vanishing Gradient \parencite{Bengio1994}}
        \footnotesize
    ...
    \begin{column}{0.42\textwidth}
      ...
      \medskip
      \begin{keybox}
        \textbf{The fix:} LSTM introduces gating mechanisms that
...
\begin{frame}{Section Overview}
  \begin{keybox}
    \textbf{LSTM} \parencite{Hochreiter1997} solves the vanishing gradient
    problem by introducing a \emph{cell state} $\mathbf{c}_t$ --- a
    protected information highway --- and three \emph{gates} that control
    what to forget, store, and output at each time step.
  \end{keybox}
\end{frame}
...
% -----------------------------------------------------------------------------
  \begin{columns}[T]
    \begin{column}{0.56\textwidth}
      \begin{definitionbox}{LSTM Gate Equations \parencite{Hochreiter1997}}
        \footnotesize
        \begin{align*}
    ...
    \begin{column}{0.40\textwidth}
      \begin{examplebox}{Forget gate $f_t$}
        \footnotesize
        $f_t \approx 1$: keep memory.
        $f_t \approx 0$: erase. During a recession, the model can learn
        to reset the normal-growth signal.
      \end{examplebox}

      \smallskip
      \begin{examplebox}{Input gate $i_t$}
        \footnotesize
        Controls which new information is written to cell state.
        A large December dummy triggers high $i_t$ to write the
        holiday pattern.
      \end{examplebox}

      \smallskip
      \begin{examplebox}{Output gate $o_t$}
        \footnotesize
        Controls which part of the cell state flows to $\mathbf{h}_t$
...
% -----------------------------------------------------------------------------
  \begin{columns}[T]
    ...
    \begin{column}{0.44\textwidth}
      ...
      \begin{warningbox}
        \footnotesize
        The first $T$ observations are consumed to build the initial
        sequence --- they cannot be used as targets. With $T = 24$,
        you lose the first 24 months from the training set.
        Document this shift when reporting sample sizes.
      \end{warningbox}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Section Overview}
  \begin{keybox}
    \textbf{Attention} allows the model to weight different time steps
    differently when making a prediction --- instead of compressing
    the entire past into a single fixed-size hidden state.
    The Transformer \parencite{Vaswani2017} made attention the dominant
    architecture in sequence modeling.
  \end{keybox}
\end{frame}

% -----------------------------------------------------------------------------
\begin{frame}{Attention: Letting the Model Look Back Selectively}
% -----------------------------------------------------------------------------
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      \begin{definitionbox}{Scaled Dot-Product Attention \parencite{Vaswani2017}}
        \footnotesize
        Given hidden states $\mathbf{H} = [\mathbf{h}_1, \ldots, \mathbf{h}_T]$
        and a query $\mathbf{q}$ (current step):
        \[
          \boldsymbol{\alpha}
          = \mathrm{softmax}\!\left(
        ...
        \]
        Weights $\boldsymbol{\alpha}$ sum to 1 and are learned end-to-end.
        High $\alpha_s$ means: ``attend to what happened at step $s$.''
      \end{definitionbox}

      \smallskip
      \textbf{Intuition:}
      \begin{itemize}
        \item High weight on step $t-12$ = attending to
              ``same month last year''
        \item High weight on $t-1$ = short-run momentum
        \item Attention \emph{learns} which lags matter; no manual
    ...
    \begin{column}{0.44\textwidth}
      \begin{examplebox}{What Attention Solves}
        ...
        updates. It can get diluted. Attention \emph{directly} reads the
        value at step $t-12$ with weight $\alpha_{t-12}$ ---
      ...
      \begin{warningbox}
        \footnotesize
        For monthly retail sales ($n \approx 300$, $T = 24$), attention
        adds modest benefit over LSTM. Its main advantage is in very long
        sequences (daily data, $n > 5000$) or when interpretability of
        time-step importance is needed.
      \end{warningbox}
    \end{column}
  \end{columns}
\end{frame}
...
% -----------------------------------------------------------------------------
  ...
  \muted{\footnotesize\itshape
    Socratic: self-attention has no built-in sense of order ---
    it sees an unordered set of hidden states.
...
\begin{frame}{Section Overview}
  \begin{keybox}
    We implement a two-layer LSTM forecaster using Keras (TensorFlow
    backend). The key data engineering step is reshaping the flat feature
    matrix into 3-D sequences:
...
\begin{frame}{Section Overview}
  \begin{keybox}
    Apply the two-layer LSTM to RSXFS retail sales and compare against
    the full Lectures 01--09 leaderboard. Key question: does LSTM justify
    its added complexity on $n \approx 300$ monthly observations?
  \end{keybox}
\end{frame}
  ...
  \begin{columns}[T]
    \begin{column}{0.52\textwidth}
      \begin{warningbox}
        ...
        A two-layer LSTM with 64 units has $\sim$50,000 parameters.
